...
export async function scanSWAAndReport(swa: SWA) {
	...
	enhancedSWA.defaultHostname = await scanDomain(
		enhancedSWA.name,
		enhancedSWA.defaultHostname,
	);
	const customDomains = [];
...
export async function scanDomain(name: string, domain: SecureDomain) {
	...
	const wafResult = await exec(
		`wafw00f https://${domain.domain} -f json -o ${name}.${domain.domain}.json`,
	);
	if (wafResult.stderr) {
	...
	const domainResult: {
		...
		firewall: string;
		manufacturer: string;
	} = JSON.parse(readFileSync(`${name}.${domain.domain}.json`, "utf8"))[0];

	//Try to curl to get the status code
	const httpStatus = await exec(
		`curl -s -o /dev/null -w "%{http_code}" --location https://${domain.domain}`,
	);
